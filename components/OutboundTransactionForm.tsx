...
export const OutboundTransactionForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('تراکنش خروجی با موفقیت ثبت شد.', 'success');
      setQuantity('');
      setPassword('');
      setDescription('');
    } catch (err: any) {
      ...
      <Card title="ثبت خروجی کالا">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:bg-red-400"
          >
            {isLoading ? (
